...
LoadBalancingListener::LoadBalancingListener(BaseView* pView) : mpLbc(NULL), mpView(pView), mLocalPlayerNr(0), mGridSize(DEFAULT_GRID_SIZE), mMap(DEFAULT_MAP), mAutomove(true), mUseGroups(true), mSendGroup(0), mConnected(false)
{
}
...
void LoadBalancingListener::setLBC(Client* pLbc)
{
	this->mpLbc = pLbc;
}

void LoadBalancingListener::connect(const JString& userName, const JString& serverAddress)
{
	ConnectOptions connectOptions = ConnectOptions().setAuthenticationValues(AuthenticationValues().setUserID(userName)).setUsername(userName).setTryUseDatagramEncryption(true);
	if(serverAddress.length())
		connectOptions.setServerAddress(serverAddress);
	mpLbc->connect(connectOptions);
	UE_LOG(LogTemp, Log, TEXT("Connect!!!!!!!!!!!!!!"));
}

void LoadBalancingListener::disconnect(void)
{
...
#ifdef __UNREAL__
			mpView->changePlayerPos(playerNr, mGridSize-1-y, x); // convert from demo to engine coordinate system on receive (necessary so that builds of this demo for different game engines are compatible to each other)
#else
...
{
	UE_LOG(LogTemp, Log, TEXT("ConnectReturn!!!!!!!!!!!!!!!!!!!"));
	updateState();
...
void LoadBalancingListener::disconnectReturn(void)
{
	...
	UE_LOG(LogTemp, Log, TEXT("DisconnectReturn!!!!!!!!!!!"));
	mpView->initPlayers();
	mConnected = false;
}
...
{
	updateState();
	UE_LOG(LogTemp, Log, TEXT("CreateRoomReturn!!!!!!!!!!!!!!"));
	if(errorCode == ErrorCode::OK)
	{
...
void LoadBalancingListener::joinLobbyReturn(void)
{
	...
	UE_LOG(LogTemp, Log, TEXT("JoinLobbyReturn!!!!!!!!!!!!!!"));
	updateState();
}
...
void LoadBalancingListener::leaveLobbyReturn(void)
{
	...
	UE_LOG(LogTemp, Log, TEXT("LeaveLobbyReturn!!!!!!!!!!!!!!"));
	updateState();
}
...
bool LoadBalancingListener::updateGridSize(const ExitGames::Common::Hashtable& props)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UpdateGridSize : %d"), mGridSize);
	return false;
}
...
void LoadBalancingListener::createRoom()
{
	...
	UE_LOG(LogTemp, Log, TEXT("Creating room %s"), *FString(name.cstr()));
}
...
void LoadBalancingListener::changeRandomColor(void)
{
	...
	raiseColorEvent();
	mpView->changePlayerColor(mLocalPlayerNr, mLocalPlayer.color);
	UE_LOG(LogTemp, Log, TEXT("ChangeRandomColor : %d!!!!!!!!!!!!!!!!!!"), mLocalPlayer.color);
}
...
void LoadBalancingListener::nextGridSize(void)
{
	...
	UE_LOG(LogTemp, Log, TEXT("NextGridSize : %d!!!!!!!!!!!!"), mGridSize);
...
void LoadBalancingListener::updateGroups(void)
{
	if(mpLbc->getIsInRoom())
	{
		...
		if(mUseGroups)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UpdateGroups : %d!!!!!!!!!!!!"), mUseGroups);
		}
		else {
