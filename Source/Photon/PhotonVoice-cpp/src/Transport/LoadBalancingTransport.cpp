...
namespace ExitGames
{
	namespace Voice
	{
		...
		{
			Common::ValueObject<Common::Object*> content = mpProtocol->buildVoicesInfo(voices, true);
		...
		{
			Common::ValueObject<Common::Object*> content = mpProtocol->buildVoiceRemoveMessage(voice);
		...
		void LoadBalancingTransport::sendFrameRaiseEvents(void)
		{
			...
			while(!empty) // Dequeue and process while the queue is not empty
			{
				...
				if(!empty)
				{
					...
					nByte* pContent = allocateArray<nByte>(p.mData.getSize()+4);
					pContent[0] = 4;
					pContent[1] = p.mVoiceId;
					pContent[2] = p.mEvNumber;
					pContent[3] = 0;
					MEMCPY(pContent+4, p.mData.getArray(), p.mData.getSize());
					if(mDirect)
						mLBClient.sendDirect(pContent, p.mData.getSize()+4, SendDirectOptions().setFallbackRelay(true).setReceiverGroup(p.mDebugEchoMode?Lite::ReceiverGroup::ALL:Lite::ReceiverGroup::OTHERS));
					else
					{
						//Encrypt = localVoice.Encrypt
						mLBClient.opRaiseEvent(p.mReliable, pContent, p.mData.getSize()+4, EventCode::VOICE_FRAME_DATA, RaiseEventOptions().setChannelID(p.mChannelId).setReceiverGroup(p.mDebugEchoMode?Lite::ReceiverGroup::ALL:Lite::ReceiverGroup::OTHERS).setInterestGroup(p.mInterestGroup));
		...
		{
			mpProtocol->onVoiceFrameEvent(*mpVoiceClient, eventContent, VOICE_CHANNEL, playerNr, mLBClient.getLocalPlayer().getNumber());
		}
