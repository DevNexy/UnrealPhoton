...
namespace ExitGames
{
	namespace Voice
	{
		...
		ValueObject<Object*> PhotonTransportProtocol::buildVoicesInfo(const JVector<const LocalVoice*>& voicesToSend, bool logInfo) const
		{
			...
			Object content[] =
			{
				...
				ValueObject<Object*>(infos, static_cast<int>(voicesToSend.getSize())),
			};
		...
		void PhotonTransportProtocol::onVoiceFrameEvent(VoiceClient& voiceClient, const Object& content, int channelId, int playerId, int localPlayerId)
		{
			...
			if(size < 3)
			{
				mLogger.logError(L"onVoiceFrameEvent did not receive data (readable as byte[])");
				return;
			}
			...
			Buffer<nByte> receivedBytes(size-dataOffset);
